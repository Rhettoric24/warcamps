...
import { CONSTANTS, BUILDING_DATA, FABRIAL_DATA } from '../core/constants.js';
...
function updateModalsUI(gameState) {
    ...
    updateModalStats({
        ...
        arenaDaily: `${5 - gameState.state.arena.dailyDuels}/5`,
...
export function updateTimeUI(gameState) {
    const elapsed = Date.now() - gameState.state.startTime;
    const totalDays = Math.floor(elapsed / CONSTANTS.DAY_MS);
    ...
    const msNext = CONSTANTS.DAY_MS - (elapsed % CONSTANTS.DAY_MS);
...
export function updateMilitaryUI(gameState) {
    ...
    document.getElementById('arena-daily').innerText = `${5 - gameState.state.arena.dailyDuels}/5`;
